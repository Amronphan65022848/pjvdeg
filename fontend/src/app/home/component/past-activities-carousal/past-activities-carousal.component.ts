...
@Component({
  selector: 'app-past-activities-carousal',
  templateUrl: './past-activities-carousal.component.html',
  styleUrls: ['./past-activities-carousal.component.css']
})
export class PastActivitiesCarousalComponent implements OnInit {
  ...
  filterActivitiesByDate(activities: any[]): any[] {
    ...
    return activities.filter(activity => {
      const endDate = new Date(activity.endDate); // แปลง startDate เป็น Date object// แปลง endDate เป็น Date object
      return today > endDate ; // ตรวจสอบว่าวันนี้อยู่ในช่วงเวลา
    });
  }
